"""
External validation pipeline for TabPFN.

This script trains TabPFN on the full internal training dataset and evaluates it on
one or more external or holdout datasets.

Design goals:
- GitHub-ready CLI script
- no hard-coded paths or credentials
- leakage-safe thresholds from internal out-of-fold predictions only
- optional Platt/isotonic calibration fitted only within internal training data
- reusable metrics/bootstrap/calibration outputs
- FP/FN tables for error analysis
- supports multiple external datasets in one run

Example:
    python scripts/external_validate_tabpfn.py \
        ...
        --output-dir artifacts/models/tabpfn_external_validation \
...
from the internal dataset and required in every external dataset.
...
def parse_named_paths(values: Sequence[str]) -> Dict[str, Path]:
    """Parse CLI values like name=path or plain path."""
    out: Dict[str, Path] = {}
    for item in values:
        if "=" in item:
...
def infer_feature_columns(
    ...
    *,
    ...
    """
    Infer TabPFN features.

    TabPFN can consume categorical/string columns, so the default inference keeps
    numeric, boolean, object, string, and categorical columns. ID columns and the
    target are excluded. Columns that are entirely missing are skipped.
    """
    if feature_cols:
    ...
    for col in candidate_cols:
        ...
        if is_supported and not series.isna().all():
            inferred_cols.append(col)

    if not inferred_cols:
        raise ValueError(
            "No usable feature columns found. Pass --feature-cols explicitly. "
            "Supported inferred dtypes: numeric, bool, object, string, category."
        )
...
def prepare_dataset(
    ...
    *,
    ...
    # Preserve categorical information for TabPFN instead of forcing numeric-only inputs.
...
def bootstrap_calibration(y_true: np.ndarray, y_prob: np.ndarray, *, n_boot: int, seed: int) -> pd.DataFrame:
    ...
    for metric in ["brier", "citl", "slope"]:
        if metric not in boot.columns:
            summary.append({"metric": metric, "point_estimate": np.nan, "bootstrap_mean": np.nan, "ci_low": np.nan, "ci_high": np.nan})
            continue
        mean, low, high = ci(boot[metric])
...
def make_tabpfn_classifier(*, device: str, tabpfn_version: str) -> TabPFNClassifier:
...
def run_external_validation(args: argparse.Namespace) -> None:
    ...
    X_internal, y_internal, internal_metadata = prepare_dataset(
        internal,
        features=features,
        target_col=args.target_col,
        id_cols=args.id_cols,
    ...
    LOGGER.info("Fitting TabPFN on full internal data: %d rows, %d features", len(X_internal), len(features))
    base, cal_platt, cal_iso = fit_tabpfn_with_calibration(
        X_internal,
        y_internal,
    ...
    internal_oof_prob = oof_predict_proba_tabpfn(
        X_internal,
        y_internal,
        calibration_mode=config.calibration_mode,
    ...
    threshold_values = optimal_thresholds(y_internal.to_numpy(), internal_oof_prob)
    thresholds = {
        "default": 0.5,
        "mcc": threshold_values["mcc"],
        "youden": threshold_values["youden"],
        "f1": threshold_values["f1"],
    }
    pd.DataFrame([thresholds]).to_csv(output_dir / "thresholds_from_internal_oof.csv", index=False)

    internal_oof_predictions = pd.DataFrame(
        {
            "row_index": X_internal.index,
            "dataset": "internal_oof_threshold_source",
            "y_true": y_internal.to_numpy(dtype=int),
            "p": internal_oof_prob,
        }
    )
    internal_oof_predictions.to_csv(output_dir / "internal_oof_probabilities_for_thresholds.csv", index=False)

    all_predictions = []
    all_metrics = []
    all_calibration = []

    external_paths = parse_named_paths(args.external_data)
    for dataset_name, dataset_path in external_paths.items():
        ...
        p_ext = predict_tabpfn_probabilities(
            base,
            cal_platt,
            cal_iso,
            X_ext,
        ...
        plot_roc_curve(y_np, p_ext, dataset_dir / "roc_curve.png", title=f"ROC Curve – {dataset_name}")
        plot_pr_curve(y_np, p_ext, dataset_dir / "precision_recall_curve.png", title=f"Precision–Recall Curve – {dataset_name}")
        plot_calibration_curve(y_np, p_ext, dataset_dir / "calibration_curve.png", title=f"Calibration Curve – {dataset_name}")
    ...
    metadata = {
        ...
        "external_data": {name: str(path) for name, path in external_paths.items()},
        "thresholds": thresholds,
    ...
    if args.save_model:
        joblib.dump({"base": base, "cal_platt": cal_platt, "cal_iso": cal_iso, "features": features, "thresholds": thresholds}, output_dir / "tabpfn_external_validation_model.joblib")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train TabPFN on internal data and externally validate on one or more datasets.")
    ...
    parser.add_argument("--output-dir", default="artifacts/tabpfn_external_validation")
